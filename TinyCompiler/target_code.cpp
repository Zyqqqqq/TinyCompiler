...
int gtcnt=0;
int ltcnt=0;
void print_code(Quaternion qua)//对应不同类型，待翻译的汇编代码段 
{
...
int main()
{
	...
	streambuf *backup;  
	ifstream fin;  
	fin.open("data\\data_target_code.txt");  
	cin.rdbuf(fin.rdbuf()); // assign file's streambuf to cin 
    while(1){
        cout<<"请输入四元式：以$结束";
        cin>>s_tmp;
        if(s_tmp=="$"){
            cout<<"\nmasm汇编代码已输出到文件Data\\a.asm\n\n\n\n";
            break; 
        }
    ...
    //记得修改这里的输出文件地址 
    freopen("data\\a.asm","w",stdout);  
    testCode();
    freopen("con", "w", stdout);
    Sleep(3000);
    puts("1.运行dosbox后，输入mount c: a.asm所在文件夹路径\n2.将‘a.asm所在文件夹路径’替换成绝对路径，如‘f:\\dos’\n参照截图1，2，3");
    system("pause"); 
    return 0;
}
/* 输入样例 
